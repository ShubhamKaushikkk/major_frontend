...
const Newvoter = () => {
  ...
  const { setnum } = useBetween(UseShareableState);
  ...
  const handlefingerprint = async (e) => {
    e.preventDefault();
    const body = detail;
    body.fid = body.adharNumber % 100;
    console.log(body);
    await axios
      .post("https://major-backend-ealt.onrender.com/api/v0/signup", body)
      .then((res) => {
        console.log(res);
        setnum(res.adharNumber);
        alert("Record Saved");
      })
